...
class ReportGenerator:
    ...
    def _load_prompt_template(self) -> tuple[str, str]:
        ...
        prompt_path = PROJECT_ROOT / "promt.md"
...
""".strip()
    ...
    async def generate_report(
        ...
        try:
            ...
            messages = [
                {
                    "role": "system",
                    "content": system_role or "You are an assistant that writes concise, structured daily productivity reports in Ukrainian."
                },
                {
                    "role": "user",
                    "content": user_prompt
            ...
            # Use standard chat.completions API (works with all models)
    ...
    def _extract_response_text(self, response: Any) -> str:
        ...
        # Try output_text (responses API format)
        if hasattr(response, "output_text") and response.output_text:
            return response.output_text.strip()

        # Try iterating over output blocks (responses API)
